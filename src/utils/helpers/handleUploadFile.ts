...
export const handleUploadProfilePicture = async (
  ...
  try {
    const res = await axios.post("/s3/uploadFile", {
      name: file.name,
      type: file.type,
      folder: `profile/${loggedInUser.id}/profilePicture`,
...
export const handleUploadCoverPicture = async (
  file,
  loggedInUser,
  callback
) => {
  try {
    const res = await axios.post("/s3/uploadFile", {
      name: file.name,
      type: file.type,
      folder: `profile/${loggedInUser.id}/coverPicture`,
      userId: loggedInUser.id,
    });
...
export const handleUploadBioPicture = async (file, loggedInUser, callback) => {
  try {
    const res = await axios.post("/s3/uploadFile", {
      name: file.name,
      type: file.type,
      folder: `profile/${loggedInUser.id}/bioPicture`,
      userId: loggedInUser.id,
    });
